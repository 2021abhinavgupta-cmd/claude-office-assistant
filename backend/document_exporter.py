...
<div class="title-block">
  ...
  <p>Generated by Agency Portal Assistant &middot; {datetime.datetime.now().strftime("%B %d, %Y")}</p>
</div>
{html_body}
</body></html>"""
...
def normalize_slide_markdown_for_pptx(text: str, default_title: str = "Presentation") -> str:
    """
    Convert 'Slide N: headline' prose (common from System) into ## SLIDE N: markdown.
    ...
    if idx_first is not None and idx_first > 0:
        ...
        junk = re.compile(r"^(?:✏\s*)?edit\s*$|^(?:✦)+\s*$|^claude\s*$|^.{1,6}$", re.I)
        titleish = []
        for p in raw_pre:
            if junk.match(p):
                continue
...
def _fetch_image_bytes(url: str, timeout: int = 24) -> Optional[bytes]:
    try:
        ...
        if "unsplash.com" in host:
            ...
            ref = f"https://{host}/"
        headers = {
            "User-Agent": "Mozilla/5.0 (compatible; SystemOfficeExport/1.0; +office-assistant)",
...
def _placeholder_image_bytes(seed: str) -> Optional[bytes]:
    ...
    try:
        ...
        r = req.get(u, timeout=12, headers={"User-Agent": "SystemOfficeExport/1.0"})
